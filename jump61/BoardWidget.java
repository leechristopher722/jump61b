...
class BoardWidget extends Pad {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int i = 0; i < _board.size() * _board.size(); i++) {
            if (_board.get(i).getSide().equals(RED)) {
                g.setColor(RED_TINT);
            } else if (_board.get(i).getSide().equals(BLUE)) {
    ...
    /** Displays a spot.
     * @param g Graphics with 2D
     * @param r Row of the spot
     * @param c Column of the spot
     * */
    private void displaySpots(Graphics2D g, int r, int c) {
        int x = SQUARE_SEP * (r - 1);
        int y = SQUARE_SEP * (c - 1);
        g.fillRect(x, y, SQUARE_SIZE, SQUARE_SIZE);
        Color currColor = g.getColor();
        g.setColor(currColor);

        int spotNum = _board.get(r, c).getSpots();
        if (spotNum == 1) {
            spot(g, x + SQUARE_SIZE / 2, y + SQUARE_SIZE / 2);
        } else if (spotNum == 2) {
            spot(g, x + SPOT_MARGIN, y + SPOT_MARGIN);
            spot(g, x + SQUARE_SIZE - SPOT_MARGIN,
                    y + SQUARE_SIZE - SPOT_MARGIN);
        } else if (spotNum == 3) {
            spot(g, x + SPOT_MARGIN, y + SPOT_MARGIN);
            spot(g, x + SQUARE_SIZE - SPOT_MARGIN,
                    y + SQUARE_SIZE - SPOT_MARGIN);
    ...
    public void doClick(String dummy, MouseEvent event) {
        ...
        int r = Math.floorDiv(x, SQUARE_SEP) + 1;
        int c = Math.floorDiv(y, SQUARE_SEP) + 1;
